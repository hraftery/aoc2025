...
# And finally, finally, note the following targets have this Makefile itself as a pre-req, and the
# build directory as a "order-only" pre-req. So any change to the Makefile prompts a full rebuild,
# and the BLD_DIR target is always satisfied first but changes to BLD_DIR don't trigger a rebuild.
.SECONDEXPANSION:
pc := %
$(patsubst %, $(BLD_DIR)/%, $(DAYS)): \
$(BLD_DIR)/day%: $$(patsubst $$(pc), $(BLD_DIR)/$$(pc).o, $$(DAY$$*_REQS)) Makefile | $(BLD_DIR)
	$(CC) $(filter-out Makefile,$^) -o $@ $(LDFLAGS)

# All c files are compiled to object files the same way
$(BLD_DIR)/%.o: $(SRC_DIR)/%.c Makefile | $(BLD_DIR)
...
$(BLD_DIR)/%.d: $(SRC_DIR)/%.c Makefile | $(BLD_DIR)
	@mkdir -p $(dir $@)
	set -e; rm -f $@; \
	$(CC) -MM $(CFLAGS) $< > $@.$$$$; \
